...
import { APIS, WSS, CONTRACTS, CHAIN_VERSIONS, BASIS, Network, ZIL_HASH, PELE_HASH, WHITELISTED_TOKENS } from './constants'
...
export class Zilswap {
  ...
  public async addLiquidity(
    ...
    const token = this.getTokenDetails(tokenID)
    const pele = this.getTokenDetails(PELE_HASH[this.network])
    ...
    await this.checkAllowedBalance(pele, zilsToAdd)
  ...
  public async swapWithExactInput(
    ...
    if (tokenIn.hash === PELE_HASH[this.network]) {
      // pele to zrc2
      txn = {
        transition: 'SwapExactPELEForTokens',
        args: [
          {
            vname: 'pele_amount',
            type: 'Uint128',
    ...
    } else if (tokenOut.hash === PELE_HASH[this.network]) {
      // zrc2 to pele
      txn = {
        transition: 'SwapExactTokensForPELE',
        args: [
          {
            vname: 'token_address',
            type: 'ByStr20',
  ...
  public async swapWithExactOutput(
    ...
    if (tokenIn.hash === PELE_HASH[this.network]) {
      // pele to zrc2
      txn = {
        transition: 'PELE_HASH[this.network]',
        args: [
          {
            vname: 'pele_amount',
            type: 'Uint128',
      ...
      // zrc2 to pele
      txn = {
        transition: 'SwapTokensForExactPELE',
        args: [
          {
            vname: 'token_address',
            type: 'ByStr20',
  ...
  private async loadTokenList() {
    if (this.network === Network.TestNet) {
      this.tokens['PELE'] = 'zil1l8cz5x99ynfyzmxmrc5k5z45shq8crmc46ntw6';
      this.tokens['T1'] = 'zil1j8rsd3cz29t6a0smcl6rrpmqz6qgpqzkj2ud05';
      this.tokens['T2'] = 'zil1kr9padtnvystth5rzkyx33y092xwu0xhusuaup';
